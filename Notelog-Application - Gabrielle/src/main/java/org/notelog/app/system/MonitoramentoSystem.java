...
import static org.notelog.dao.RedeWifiDAO.obterRedeWiFi;
...
public class MonitoramentoSystem {
    ...
    public static void escolherMonitoramento(Funcionario usuario, Notebook notebook) throws InterruptedException {
        ...
        do {
            ...
            System.out.println("7 - Monitorar rede Wifi");
    ...
    public static void monitorarRedeWifi() {
        RedeInterface wifi = obterRedeWiFi();
        if (wifi != null) {
            RedeWifi redeWifi = new RedeWifi();
            redeWifi.setNome(wifi.getNome());
            redeWifi.setNomeExibicao(wifi.getNomeExibicao());
            redeWifi.setEnderecoIpv4(wifi.getEnderecoIpv4().toString());
            redeWifi.setEnderecoIpv6(wifi.getEnderecoIpv6().toString());
            redeWifi.setEnderecoMac(wifi.getEnderecoMac());
            redeWifi.setBytesRecebidos(wifi.getBytesRecebidos());
            redeWifi.setBytesEnviados(wifi.getBytesEnviados());
            redeWifi.setPacotesRecebidos(wifi.getPacotesRecebidos());
            redeWifi.setPacotesEnviados(wifi.getPacotesEnviados());
            // Defina o fkNotebook de acordo com a sua lógica de negócios
            //redeWifi.setFkNotebook(1); // Exemplo de valor

            RedeWifiDAO dao = new RedeWifiDAO();
            dao.adicionarRedeWiFi(redeWifi);

            System.out.println("Dados da rede Wi-Fi inseridos no banco de dados.");
        } else {
            System.out.println("Nenhuma rede Wi-Fi conectada encontrada.");
        }
    }
