...
public class NotebookDAO {
    public Boolean adicionarNotebook(Notebook notebook) {
        ...
        int funcionarioExistente = con.queryForObject("select count(*) from Funcionario where fkEmpresa = ? and id = ?",
                Integer.class, notebook.getFkEmpresa(), notebook.getFkFuncionario());
        int funcionarioJaAtrelado = con.queryForObject("select count(*) from Notebook join Funcionario on Funcionario.id = " +
                "fkFuncionario where fkFuncionario = ?;", Integer.class, notebook.getFkFuncionario());
